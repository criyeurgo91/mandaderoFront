...
function MandersList() {
    ...
    const fetchManders = async () => {
      try {
        const response = await axios.get('http://manders.azurewebsites.net/api/mander/');
        const mandersWithUserInfo = await Promise.all(
          response.data.map(async (mander) => {
            const userResponse = await axios.get(`http://manders.azurewebsites.net/api/user/${mander.user_id_user}/`);
            return {
              ...mander,
    ...
    const handleDeleteMander = async (manderId) => {
      try {
        await axios.delete(`http://manders.azurewebsites.net/api/mander/${manderId}/`);
        fetchManders();
      } catch (error) {
