...
const TotalManders = () => {
  const [mandersTotal, setMaderTotal] = useState(0);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setMaderTotal(data.total_manders);
      } catch (error) {
  ...
  const percentage = (mandersTotal / 100) * 100; 

  return (
    <Card className="p-6 bg-white rounded-lg shadow-md max-w-sm transform hover:scale-105">
      <Text className="text-lg font-semibold text-gray-700">Total Mandaderos</Text>
      <Metric className="text-4xl font-bold text-blue-600">{mandersTotal}</Metric>
